...
def read_datamy(args):
    train_data, train_lable = load_data(r'D:\MNIST\MNIST\raw', "train-images-idx3-ubyte.gz",
                                        "train-labels-idx1-ubyte.gz")
    test_data, test_lable = load_data(r'D:\MNIST\MNIST\raw', "t10k-images-idx3-ubyte.gz",
...
def load_partition_data_mnist(args):
    ...
    print("train_data大小", len(train_data), "test_data大小", len(test_data))
    if len(groups) == 0:
...
#     users, groups, train_data, test_data, data_size_bid, num_sample_data = read_data(train_path, test_path)  # 读数据函数return中加了data_size_bid,冲突了
#     print("train_data大小", len(train_data), "test_data大小", len(test_data))
#     if len(groups) == 0:
#         groups = [None for _ in users]
#     train_data_num = 0
...
#     client_idx = 0
#     logging.info("loading data...")
#     # print("user的len", len(users), "groupsde", len(groups))
#     # print("2222222222222222222", train_data[users[0]])
#     # print("--------------------", train_data["f_00000"])
#     for u, g in zip(users, groups):
#         user_train_data_num = len(train_data[u]['x'])  # train_data[u]-----['f_00000', 'f_00001', 'f_00002'
#         user_test_data_num = len(test_data[u]['x'])
#         # print("user_train_data_num_daxiao",user_test_data_num, "user_test_data_num",user_test_data_num)
