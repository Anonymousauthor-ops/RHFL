...
class FedAvgAPI(object):
    ...
    def train(self):
        ...
        # malicious_clients = [1, 2, 0,3,4]  # 认为控制m的列表
        ...
        for round_idx in range(self.args.comm_round):
            ...
            for model_index, model_name in enumerate(self.args.task_array):
                ...
                # w_global = loadmodel.cpu().state_dict()
                ...
                # # 保存列表到文件
                # # self.save_list_to_file(w_locals, 'd:/canshu.txt')
                # # print('w_locals的类型',type(w_locals))
                ...
                w_global = self._aggregate(w_locals_id2)
                # 简单平均聚合策略 CS端
            ...
            # print('ES_global长度------', ES_global)HFL
            # 计算每个ES端全局模型的测试acc,并处理为聚合的权重
            # CS_global = self._aggregate_CS(ES_global)   #简单平均聚合
    ...
    def compute_threshold(self, calculate_kl_diverg, client_indexes):
        ...
        if all(math.floor(math.log10(abs(value))) == first_magnitude for _, value in calculate_client_kl1_sorted):
            ...
            # 客户端的值的量级不相同
            # threshold = 10 ** (first_magnitude - 1)
    ...
    def save_test_acc(self, test_acc):
        ...
        df.to_csv('./csv/ours-LF-n50_k_3_' + '_M_' + str(m) + 'mnist_.csv', index=None)
